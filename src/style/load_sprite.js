...
function parseBinaryManifest(data: ArrayBuffer) {
    return new Protobuf(data).readFields(function(tag: number, manifest: Object, pbf: Protobuf) {
        if (tag === 1) {
            let id;
            const img = pbf.readMessage(function(tag: number, img: Object, pbf: Protobuf) {
